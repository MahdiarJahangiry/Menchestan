    ...
    namespace Audio
    {
        public class AudioManager : MonoBehaviour
        {
            ...
            public static AudioManager Instance
            {
                get
                {
                    if (instance == null)
                    {
                        ...
                        if (instance == null)
                        {
                            instance = new GameObject("AudioManager", typeof(AudioManager)).AddComponent<AudioManager>();
                        }
                        instance.Configure();
                    }
                    return instance;
            ...
            public bool SfxState
            {
                ...
                {
                    sfxState = value;
                    PlayerPrefs.SetInt("sfxState", value ? 1 : 0);
                    tracks[1].source.mute = !sfxState;
                }
            }
            public bool SoundTrackState
            {
                get => soundTrackState;
                set
                {
                    soundTrackState = value;
                    PlayerPrefs.SetInt("soundTrackState", value ? 1 : 0);
                    tracks[0].source.mute = !soundTrackState;
                }
            }
            public bool debug;

            private bool sfxState;
            private bool soundTrackState;
            private AudioOneShot oneShot;
            ...
            private void Awake()
            {
                Instance.SoundTrackState = PlayerPrefs.GetInt("soundTrackState", 1) == 0 ? false : true;
                Instance.SfxState = PlayerPrefs.GetInt("sfxState", 1) == 0 ? false : true;
            }
            private void Start()
            {
                PlayAudio(AudioType.ST_Menu);
            }
            ...
            public void PlayInstanciateAudio(AudioType _type, bool _fade = false, float _delay = 0.0F)
            {
                AudioOneShot oneShotAudio = Instantiate(oneShot, transform);
                oneShotAudio.name = _type.ToString();
                oneShotAudio.audioSource.clip = GetAudioClip(_type);
                oneShotAudio.gameObject.SetActive(true);
            }
            ...
            private void Configure()
            {
                ...
                GameObject newGame = new GameObject("OneShot");
                newGame.SetActive(false);
                oneShot = newGame.AddComponent<AudioOneShot>();
                oneShot.audioSource = oneShot.GetComponent<AudioSource>();
                oneShot.transform.parent = transform;
                GenerateAudioTable();
            }
            ...
            private IEnumerator RunAudioJob(AudioJob _job)
            {
                ...
                AudioTrack _track = _track = GetAudioTrack(_job.type); // track existence should be verified by now
                AudioClip _clip = GetAudioClipFromAudioTrack(_job.type, _track);
                _track.source.clip = _clip;
                switch (_job.action)
                {
            ...
            private AudioClip GetAudioClip(AudioType _type)
            {
                ...
                {
                    theClip = GetAudioClipFromAudioTrack(_type, track);
                    if (theClip != null)
                        break;
                }
                return theClip;
